...
public class IndexFiles {
	...
	public static void main(String[] argv) {
		...
		String index = "/Users/micha/Dropbox_ZHAW/Dropbox/TestLucene/";
	 ...
	 * 
	 * @param root
	 *            The root directory to start indexing.
	 * @param create
	 *            Should we create a new index?
	 * @param index
	...
	public void index(File root, boolean create, String index) {
		...
		try {
			...
			writer = new IndexWriter(dir, new StandardAnalyzer(Version.LUCENE_30), create, IndexWriter.MaxFieldLength.LIMITED);
	 ...
	 * @param create
	 *            A flag telling if we should create the index.
	 * 
	 * @throws Exception
	 *             If there is any error indexing the directory.
	 */
	private void indexDocs(File file, String index, boolean create) throws Exception {
		if (!create) { // incrementally update
			Directory dir = FSDirectory.open(new File(index));

			reader = IndexReader.open(dir); // open existing index
	...
	private void indexDocs(File file) throws Exception {
		if (file.isDirectory()) { // if a directory
			...
			if (uidIter != null) {
				...
				while (uidIter.term() != null && uidIter.term().field().equals("uid") && uidIter.term().text().compareTo(uid) < 0) {
